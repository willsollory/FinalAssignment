...
def make_graph(tesla_stock_data, tesla_revenue, Tesla):
    fig = make_subplots(rows=2, cols=1, shared_xaxes=True, subplot_titles=("Historical Share Price", "Historical Revenue"), vertical_spacing = .3)
    tesla_stock_data = tesla_data[tesla_stock_data.Date <= '2021--06-14']
    tesla_revenue = tesla_revenue[tesla_revenue.Date <= '2021-04-30']
    fig.add_trace(go.Scatter(x=pd.to_datetime(tesla_stock_data.Date), y=tesla_stock_data.Close.astype("float"), name="Share Price"), row=1, col=1)
